...
public class DynamicCompilerUtil {
	 ...
	 * @param filePath
	 *            文件或者目录（若为目录，自动递归编译）
	 *            需要编译的.java文件路径，若为文件则只编译该.java文件，若为目录则编译该目录下的所有.java文件
	 * @param sourceDir
	 *            java源文件存放目录
	 * @param targetDir
	 *            编译后class类文件存放目录
	...
	public static boolean compiler(String filePath, String sourceDir, String targetDir, DiagnosticCollector<JavaFileObject> diagnostics)
		...
		try {
			...
			/**
			 * 编译选项，即设定javac命令的执行参数。
			 * javac命令参数：-d 指定放置生成的类文件的位置 ，即.class文件的放置位置。
			 * -sourcepath选项就是定义java源文件的查找目录， 即.java源文件的查找目录。
			 * -classpath选项就是定义class文件的查找目录。在编译java文件时，编译程序会自动的去寻找java文件引用的其他的java源文件或者class。
			 * -verbose输出有关编译器正在执行的操作的消息
			 */
			Iterable<String> options = Arrays.asList("-d", targetDir, "-sourcepath", sourceDir, "-verbose");
			
			/* JavaCompiler.getTask()参数：
